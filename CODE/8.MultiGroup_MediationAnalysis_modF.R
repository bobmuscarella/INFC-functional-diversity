...
# Mediator effect
Capv_ha_log ~ c(d0,d1) * cwm_Dim1_log
...
sub.dataset<- dataset[,c('Capv_ha_log','FDis_All_log','cwm_Dim1_log','vpd','climate_classification')]
sub.dataset<- na.omit(sub.dataset)
library(piecewiseSEM)
pmultigroup <- psem(
  lm(Capv_ha_log ~ FDis_All_log + cwm_Dim1_log+vpd, sub.dataset),
...
# Mediator effect
Capv_ha_log ~ c(d0,d0) * cwm_Dim1_log
Capv_ha_log ~ c(e0,e1) * FDis_All_log
cwm_Dim1_log ~ c(a0,a1) * vpd_log
FDis_All_log ~ c(b0,b1) * vpd_log
...
png(
  "output_plot/MultiGroup_Mod_f.jpg",
  width = 10,
  height = 4.5,
  units = 'in',
  res = 300
...
print.xtable(table1, type="html", file="output_tab/MultiGroup_Mod_f.html")
