...
sub.dataset<- dataset[,c('Capv_ha_log','FDis_All_log','cwm_Dim2_log','vpd','climate_classification')]
sub.dataset<- na.omit(sub.dataset)
library(piecewiseSEM)
pmultigroup <- psem(
  lm(Capv_ha_log ~ FDis_All_log + cwm_Dim2_log+vpd, sub.dataset),
  lm(FDis_All_log ~ vpd, sub.dataset),
  lm(cwm_Dim2_log ~ vpd, sub.dataset)
...
#                                                          #
############################################################
(out <- measurementInvariance(model=modG,
                              fixed.x = FALSE,
...
png(
  "output_plot/MultiGroup_Mod_g.jpg",
  width = 10,
  height = 4.5,
  units = 'in',
  res = 300
...
print.xtable(table1, type="html", file="output_tab/MultiGroup_Mod_g.html")
