...
# Direct effect
Prod=~ g1*Capv_ha_log
Prod=~ g2*ICCapv_ha_log
Prod=~ g3* Capm_ha_log

Prod ~ c*vpd_log

# Mediator effect
Prod ~ d * cwm_SeedMass_log
Prod ~ e * FDis_SeedMass_log
...
m <- matrix(
  c(NA, 'vpd_log',  NA,
    ...
    NA, "Prod", NA,
    "Capv_ha_log", 'ICCapv_ha_log','Capm_ha_log'),
  byrow = TRUE,
  4, 3)
p_pa <- semPaths(
  fit1,
...
p_pa$graphAttributes$Nodes$width<-
  c(
    7, 7, 7, 10, 10, 10,  10
  )
...
# Mediator effect
Prod ~ d*cwm_Height_log
Prod ~ e*FDis_Height_log
...
# Mediator effect
Prod ~ d*cwm_StemDensity_log
Prod ~ e*FDis_StemDensity_log
...
# Mediator effect
Prod ~ d*cwm_XylemVulnerability_log
Prod ~ e*FDis_XylemVulnerability_log
