...
sub.dataset<- dataset[,c('ICCapv_ha_log','FDis_XylemVulnerability_log','cwm_XylemVulnerability_log','vpd','climate_classification')]
sub.dataset<- na.omit(sub.dataset)
library(piecewiseSEM)
pmultigroup <- psem(
  lm(ICCapv_ha_log ~ FDis_XylemVulnerability_log + cwm_XylemVulnerability_log+vpd, sub.dataset),
  lm(FDis_XylemVulnerability_log ~ vpd, sub.dataset),
  lm(cwm_XylemVulnerability_log ~ vpd, sub.dataset)
...
#                                                          #
############################################################
(out <- measurementInvariance(model=modE,
                              fixed.x = FALSE,
...
png(
  "output_plot/MultiGroup_Mod_e_Xylem.jpg",
  width = 10,
  height = 4.5,
  units = 'in',
  res = 300
